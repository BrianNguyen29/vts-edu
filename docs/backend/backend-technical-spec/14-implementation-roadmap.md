...
- Performance, cross-browser (Firefox/WebKit), notifications, installable PWA. **Bundle split đã ship 2026-07-02** (slice-13): 16 route page + 3 dashboard panel `React.lazy` imports qua `SuspenseRoute`, initial chunk giảm từ 511.19 kB / 147.04 kB gz xuống 335.39 kB / 106.56 kB gz, production sourcemap chuyển sang `hidden` (giữ `.map` trên disk nhưng bỏ `//# sourceMappingURL=` trong bundle public). Lazy boundary giữ layouts / auth provider / query provider tĩnh để auth redirect / role guard hoạt động ngay từ first paint. CSS `.loading-fallback` mới. No new deps; behavior không đổi. **Cross-browser matrix đã ship 2026-07-02** (slice-14): `playwright.config.ts` thêm `firefox` + `webkit` projects dưới `PLAYWRIGHT_BROWSERS=1`, default `pnpm e2e:browser` giữ chromium-only, mới `pnpm e2e:browser:all` probe WebKit host deps (libgtk-4 / libgraphene-1.0 / libxslt / libevent-2.1 / libopus / libgstallocators) và gracefully fallback chromium + firefox khi thiếu. Critical-flow locator thêm `.first()` cho robust với accumulated test data. Matrix run 38/40 pass (chromium 20/20, firefox 18/20 — 2 long-flow tests gặp Firefox-headless GPU/SWGL renderer crash trên WSL2 host, infrastructure không phải test logic). `pnpm check` xanh. **Notifications inbox + best-effort events đã ship 2026-07-02** (slice-15): `notifications` table + `internal/features/notifications` package với `Notifier` seam consumed by grading (`attempt.graded` post-recompute), assessments (`assessment.published` post-publish via `RecipientsResolver`), and resources (`resource.published` post-publish, class-scoped only, via `ClassRecipientsResolver`); notifier failures swallow + log để never roll back business tx. 3 endpoints (`GET /me/notifications`, `GET /me/notifications/unread-count`, `POST /me/notifications/{id}/read`) tenant-isolated, mark-read idempotent (`COALESCE(read_at, now())`). Frontend `NotificationBell` trong app shell, 30s `refetchInterval` polling, accessible dropdown (aria-haspopup/dialog, Escape, click-outside), per-item Mark-Read; hidden khi `isRestricted`. OpenAPI + `openapi-schema.d.ts` regenerated. Smoke `assertNotificationFlow` covers list + unread-count + assessment.published fan-out + mark-read + idempotency + cross-user ownership (403/404). `pnpm check` + `pnpm e2e:smoke` + `pnpm e2e:browser` (20/20) all green. **PWA Level 0 installability (manifest only) đã ship 2026-07-02** (slice-16): `apps/web/public/manifest.json` (name/short_name "VTS EDU", `start_url`/`scope`/`id` `/app`, `display: standalone`, `theme_color` `#1d4ed8`, `lang: vi`) + 2 SVG icons (`icons/icon.svg` purpose `any`, `icons/icon-maskable.svg` purpose `maskable`); `apps/web/index.html` thêm `<link rel="manifest">`, `<link rel="apple-touch-icon">`, `<meta name="theme-color">`, `application-name`, `apple-mobile-web-app-capable=yes`, `mobile-web-app-capable=yes`; favicon cũ `/vite.svg` thay bằng `/icons/icon.svg`. **Không có service worker**: zero hits cho `serviceWorker|workbox|pwa-register` trên `apps/web/{src,index.html,vite.config.ts,public}`. Không `vite-plugin-pwa`, không Workbox, không cache API/asset, không background sync / push. `apps/web/package.json` dependency surface không đổi. `pnpm web:typecheck` + `pnpm web:build` (bundle sizes không đổi — 360.50 kB / 114.37 kB gz cho `index-*.js`) + `pnpm e2e:browser` (chromium 20/20) + `pnpm check` xanh. Exam offline draft resilience vẫn ở app-layer IndexedDB (đã ship). **Exam heartbeat / deadline hardening đã ship 2026-07-02** (slice-18): `AnswerSaved` trả thêm `server_time` (required) + `expires_at` (optional) để client recalibrate countdown mà không cần round-trip riêng; `GET /attempts/{id}` hiện hữu được reuse làm 60s heartbeat (không thêm endpoint mới) refresh offset + status + expires_at khi attempt `IN_PROGRESS` (failures nuốt im lặng, không auto-submit, inputs/submit vẫn disabled khi `timeLeft=0`). Frontend duy trì `serverTimeOffsetRef` calibrate từ initial `getAttempt` và mỗi `saveAnswer` response, countdown đọc `Date.now() + serverTimeOffset` nên clock drift / stale tab không thể steal time. Deadline warning UX: `≤5 min` warning banner (`role="status"`, amber) + `≤1 min` critical banner (`role="alert"`, red, gentle pulse); chỉ transition qua ngưỡng announce (không every-tick `aria-live`); timer chip recolor. OpenAPI `SaveAnswerResponse` extended + `openapi-schema.d.ts` regenerated. Smoke `saveAnswerForAttempt` assert `server_time` + `expires_at` trên mỗi save. `pnpm api:types` + `pnpm check` + `pnpm e2e:smoke` + `pnpm e2e:browser` (23/23) all green. SW registration, real PNG icons (192/512), splash/startup images, install-prompt UI, push, background sync, bundle analyzer dependency / virtualized lists / CSS splitting / GitHub Actions matrix workflow / SSE / WebPush / outbox / notification preferences / retention purge / admin notification center vẫn deferred.
