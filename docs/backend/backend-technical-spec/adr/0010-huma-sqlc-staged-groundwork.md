...
Mỗi open issue cần một spike nhỏ trước khi runtime migration toàn cục được phê duyệt. Spoke tiếp theo đề xuất: bounded spike trên streaming/resources download để kiểm tra Huma với `http.Flusher`/SSE trước khi đề xuất full adoption.
