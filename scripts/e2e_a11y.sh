#!/usr/bin/env bash
# Bounded WCAG/axe accessibility gate runner.
#
# Spins up the same DB + API as scripts/e2e_browser.sh, then runs only
# the @axe-tagged spec against the Chromium project. The axe spec
# (apps/web/e2e/axe.spec.ts) is intentionally NOT part of `pnpm check`
# because it needs a running database and API, just like the other
# E2E flows (pnpm e2e:browser, pnpm e2e:load).
#
# Manual developer check, not CI.
set -euo pipefail
...
echo "Running axe accessibility gate (Chromium only)..."
