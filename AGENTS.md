...
pnpm e2e:a11y       # optional, manual: bounded WCAG/axe gate (Chromium only) on stable routes — NOT in `pnpm check` (see `apps/web/e2e/axe.spec.ts` and `scripts/e2e_a11y.sh`)
pnpm e2e:load       # optional, manual: bounded load / concurrency harness for attempt runtime (NOT in `pnpm check`)
pnpm e2e:load:direct # optional, manual: same harness, against an already-running API on $API_BASE
...
- Optional load / concurrency harness for attempt runtime: `scripts/e2e_load.sh` + `scripts/e2e_load_api.mjs` + root scripts `e2e:load` (full DB + API + harness) and `e2e:load:direct` (against an already-running API on `$API_BASE`). 4 bounded scenarios (N = 8–16): concurrent saves on same attempt/item (asserts all 200, revision sequence contiguous and ≥ N); concurrent submits (asserts all 200 + shared `submitted_at`/`score`/`grading_status` proving idempotency); save-after-submit (asserts 409 `attempt_not_in_progress`); burst reads (asserts all 200 + consistent status + item count). Built-in `node:fetch` only, no new dependencies, `RATE_LIMIT_ENABLED=false` for the harness. **Intentionally NOT wired into `pnpm check`** — manual developer check, not CI; uses local E2E Postgres on port 5434. `pnpm check` + `pnpm e2e:load` + `pnpm e2e:load:direct` (against temp API on 8081) all green.
- Bounded WCAG/axe accessibility gate (P2): `@axe-core/playwright@^4.12.1` added as a devDependency under `apps/web/`. New serial spec `apps/web/e2e/axe.spec.ts` runs axe-core (tags `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa`/`best-practice`) on 8 stable surfaces — `/login`, `/error/403`, `/this-does-not-exist`, `/app/student`, `/app/teacher`, `/app/resources`, `/app/change-password`, `/app/admin` — and fails the run on any actionable `violations[]`. `color-contrast` and `target-size` rules are explicitly disabled (palette + click-target sizes are tracked separately, not stable enough to gate on). New `scripts/e2e_a11y.sh` + root script `pnpm e2e:a11y` spins up the same DB + API + Vite as `e2e_browser.sh` and runs only the axe spec. **Intentionally NOT wired into `pnpm check`** (mirrors `pnpm e2e:load`/`pnpm e2e:browser` — needs a running DB/API). First run: 4/4 axe tests pass on the existing surfaces (no in-scope violations found). `pnpm web:typecheck` + `pnpm web:build` + `pnpm web:test` (57/57) + `pnpm e2e:browser` (27/27, includes 4 new axe tests) + `pnpm check` all green.
